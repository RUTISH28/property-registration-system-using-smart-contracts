...
if(action!=null && accId!=null){
  try{
    ...
    if("set_default".equals(action)){
      st.executeUpdate("UPDATE buyer_account SET status='Inactive' WHERE buyer_aadhar='"+A_Name+"'");
      st.executeUpdate("UPDATE buyer_account SET status='Active' WHERE buyer_acc_id='"+accId+"' AND buyer_aadhar='"+A_Name+"'");
      session.setAttribute("msg","Account set as default successfully.");
    } 
    else if("set_inactive".equals(action)){
      st.executeUpdate("UPDATE buyer_account SET status='Inactive' WHERE buyer_acc_id='"+accId+"' AND buyer_aadhar='"+A_Name+"'");
      session.setAttribute("msg","Account set to inactive.");
    } 
    else if("set_normal".equals(action)){
      st.executeUpdate("UPDATE buyer_account SET status='Normal' WHERE buyer_acc_id='"+accId+"' AND buyer_aadhar='"+A_Name+"'");
...
ResultSet rsA=dbA.Select(
  "SELECT buyer_acc_id,Card_Type,Card_Brand,Cardholder_Name,Card_Number,Expire_Date,Cvv,Pin,Amount,status,buyer_email "+
  "FROM buyer_account WHERE buyer_aadhar='"+A_Name+"' ORDER BY CASE status WHEN 'Active' THEN 0 WHEN 'Normal' THEN 1 ELSE 2 END, buyer_acc_id ASC");

/* count summary */
DB dC1=new DB(); ResultSet rC1=dC1.Select("SELECT COUNT(*) c FROM buyer_account WHERE buyer_aadhar='"+A_Name+"'");
int totalAcc=0; if(rC1.next()) totalAcc=rC1.getInt("c");
DB dC2=new DB(); ResultSet rC2=dC2.Select("SELECT COUNT(*) c FROM buyer_account WHERE buyer_aadhar='"+A_Name+"' AND status='Active'");
int activeAcc=0; if(rC2.next()) activeAcc=rC2.getInt("c");

/* running balance */
DB dBal=new DB(); ResultSet rBal=dBal.Select("SELECT SUM(Amount) total FROM buyer_account WHERE buyer_aadhar='"+A_Name+"' AND status='Active'");
...
<jsp:include page="buyer_sidebar.jsp" />
...
<div class="by-page">
  ...
  while(rsA.next()){
    ...
    String accEmail=rsA.getString("buyer_email"); if(accEmail==null) accEmail="";
    String aid=rsA.getString("buyer_acc_id");
  ...
  <div class="by-fade" style="margin-bottom:1.2rem">
    <div class="by-account-card <%=isActive?"is-default":isInactive?"is-inactive":""%>">
      ...
      <div class="by-acc-actions">
        <%if(!isActive){%>
        <a href="buyer_accounts.jsp?act=set_default&acc_id=<%=aid%>"
        ...
        <%}%>
        <%if(!isInactive){%>
        <a href="buyer_accounts.jsp?act=set_inactive&acc_id=<%=aid%>"
        ...
        <a href="buyer_accounts.jsp?act=set_normal&acc_id=<%=aid%>"
