...
<head>
  ...
  <title>Approved Lands — Innovative Residence</title>
...
<aside class="sd-sidebar" id="sidebar">
  ...
  <nav class="sd-nav">
    ...
    <div class="sd-nav-section">
      ...
      <a href="seller_landapprove.jsp" class="active"><i class="fa fa-check-circle-o"></i> Approved Lands</a>
...
<header class="sd-topbar">
  <div class="sd-page-title">
    Approved Lands
    <span><a href="sellerhome.jsp" style="color:var(--primary)">Dashboard</a> &rsaquo; Approved Lands</span>
  </div>
  <div class="sd-topbar-actions">
    <a href="sellerhome2.jsp" class="sd-btn sd-btn-primary sd-btn-sm"><i class="fa fa-plus"></i> Upload Land</a>
...
<main class="sd-main">
  <div class="sd-page-wide" style="padding:2rem">
    ...
    <!-- Stats -->
    <%
      DB dbC1 = new DB(); ResultSet cAll = dbC1.Select("SELECT COUNT(*) AS cnt FROM upload WHERE A_Name='" + A_Name + "'");
      int cAllCount = 0; if(cAll.next()) cAllCount = cAll.getInt("cnt");
      DB dbC2 = new DB(); ResultSet cAppr = dbC2.Select("SELECT COUNT(*) AS cnt FROM upload WHERE A_Name='" + A_Name + "' AND sts1='Approved'");
      int cApprCount = 0; if(cAppr.next()) cApprCount = cAppr.getInt("cnt");
      DB dbC3 = new DB(); ResultSet cPend = dbC3.Select("SELECT COUNT(*) AS cnt FROM upload WHERE A_Name='" + A_Name + "' AND sts1='Pending'");
      int cPendCount = 0; if(cPend.next()) cPendCount = cPend.getInt("cnt");
    %>
    <div class="sd-stats-grid" style="margin-bottom:1.5rem">
      <div class="sd-stat-card">
        <div class="sd-stat-icon sd-si-sky"><i class="fa fa-map-o"></i></div>
        <div><div class="sd-stat-n"><%=cAllCount%></div><div class="sd-stat-l">Total Lands Uploaded</div></div>
      </div>
      <div class="sd-stat-card">
        <div class="sd-stat-icon sd-si-green"><i class="fa fa-check-circle"></i></div>
        <div><div class="sd-stat-n"><%=cApprCount%></div><div class="sd-stat-l">Approved Lands</div></div>
      </div>
      <div class="sd-stat-card">
    ...
    <div class="sd-card">
      <div class="sd-card-head">
        <div>
          <div class="sd-card-title"><i class="fa fa-leaf"></i> Land Listings</div>
          <div class="sd-card-sub">All your uploaded land plots with admin approval status</div>
        </div>
      </div>
      <div class="sd-table-wrap">
        <table class="sd-table">
          ...
          <tbody>
            <%
              DB dbLand = new DB();
              ResultSet landRs = dbLand.Select("SELECT * FROM upload WHERE A_Name='" + A_Name + "' ORDER BY S_ID DESC");
              boolean hasRows = false;
              while(landRs.next()) { hasRows = true;
                String sts  = landRs.getString("sts");
                String sts1 = landRs.getString("sts1");
            ...
            %>
            <tr>
              <td style="color:var(--text3);font-size:.75rem;font-weight:600"><%=landRs.getString("S_ID")%></td>
              <td><img class="sd-table-img" src="servlet_3.jsp?name=<%=landRs.getInt("S_ID")%>" alt="Land"></td>
              <td><div class="sd-table-name"><%=landRs.getString("S_Name")%></div></td>
              <td style="font-size:.8rem"><%=landRs.getString("S_Number")%></td>
              <td style="font-size:.78rem;color:var(--text2)"><%=landRs.getString("S_MAIL")%></td>
              <td style="font-size:.83rem"><%=landRs.getString("city")%></td>
              ...
              <td style="font-size:.78rem;color:var(--text2)"><%=landRs.getString("SUNO") != null ? landRs.getString("SUNO") : "—"%></td>
              <td style="font-size:.78rem;color:var(--text2)"><%=landRs.getString("D_NO") != null ? landRs.getString("D_NO") : "—"%></td>
              <td style="font-size:.83rem;color:var(--text2)">&#8377;<%=landRs.getString("fess") != null ? landRs.getString("fess") : "—"%></td>
            ...
            <tr>
              <td colspan="14">
                <div class="sd-empty">
                  ...
                  <p>You haven't uploaded any land plots yet. <a href="sellerhome2.jsp" style="color:var(--primary)">Upload your first land</a></p>
                </div>
              </td>
            </tr>
            <% } %>
