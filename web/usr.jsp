...
<head>
  ...
  <title>Buyer Registration — Innovative Residence</title>
...
<nav class="ir-nav scrolled" id="mainNav">
  ...
  <ul class="ir-nav-links" id="navLinks">
    ...
    <li><a href="usr.jsp" class="active">Buyer Register</a></li>
  ...
  <div class="ir-nav-toggle" id="navToggle" onclick="document.getElementById('navLinks').classList.toggle('open')">
  ...
  <div class="ir-form-wrap ir-form-wrap-wide" style="max-width:680px">
    <div class="ir-form-card" style="position:relative">
      ...
      <div class="ir-form-head">
        ...
        <p>Create your buyer account. Admin approval required before login.</p>
      </div>

      
      <!-- FORM -->
      <form action="reg" enctype="multipart/form-data" method="post" onsubmit="return validateBuyer()">
        ...
        <div class="ir-row-2">
          ...
          <div class="ir-fg">
            ...
            <input type="text" id="con" name="mobile" class="ir-input"
      ...
      <div class="ir-form-foot">
        Already registered? <a href="user_login.jsp">Buyer Login &rarr;</a>
      </div>
    </div>
  </div>
</div>
...
<script>
  function togglePw(id, el) {
    ...
    if (f.type === 'password') {
      f.type = 'text';
      el.innerHTML = '<i class="fa fa-eye-slash"></i> Hide';
    } else {
      f.type = 'password';
      el.innerHTML = '<i class="fa fa-eye"></i> Show';
    }
  }
  function numOnly(e) {
  ...
  function validateBuyer() {
    ...
    if (!hname) { alert('Username cannot be empty'); return false; }
    if (!/^[a-zA-Z ]+$/.test(hname)) { alert('Username should contain only letters'); return false; }
    ...
    if (pass !== rpass) { alert('Passwords do not match'); return false; }
