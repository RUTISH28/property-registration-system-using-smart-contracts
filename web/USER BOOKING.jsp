...
/* booking: S_ID,S_NAME,S_MAIL,area,D_NO,U_NAME,U_NUMBER,U_MAIL,STS,A_ACC,C_Type,U_ACC,B_fess,key1,id,A_NAME,SUNO,SA_Name */
DB db1=new DB();
ResultSet rs=db1.Select("SELECT S_ID,S_NAME,S_MAIL,area,D_NO,SUNO,U_NAME,U_NUMBER,U_MAIL,STS,C_Type,B_fess,A_NAME,SA_Name FROM booking ORDER BY S_ID DESC");

DB dC=new DB(); ResultSet rC=dC.Select("SELECT COUNT(*) c FROM booking");
int total=0; if(rC.next()) total=rC.getInt("c");

DB dConf=new DB(); ResultSet rConf=dConf.Select("SELECT COUNT(*) c FROM booking WHERE STS='Confirmed'");
...
<head>
  ...
  <title>Land Bookings — Admin</title>
...
<header class="ad-topbar">
  <div class="ad-page-title">
    Land Bookings
    <span>booking table &mdash; <%=total%> total &middot; <%=confirmed%> confirmed</span>
  </div>
  <div class="ad-topbar-actions">
    <span class="ad-badge ad-badge-green"><%=total%> Bookings</span>
...
<div class="ad-page">
  ...
  <div class="ad-stats-grid ad-fade" style="grid-template-columns:repeat(3,1fr);max-width:500px;margin-bottom:1.5rem">
    <div class="ad-stat-card">
      <div class="ad-stat-icon ad-si-green"><i class="fa fa-calendar-check-o"></i></div>
      <div><div class="ad-stat-n"><%=total%></div><div class="ad-stat-l">Total</div></div>
    </div>
    <div class="ad-stat-card">
      <div class="ad-stat-icon ad-si-indigo"><i class="fa fa-check-double"></i></div>
  ...
  <div class="ad-card ad-fade">
    <div class="ad-card-head">
      <div>
        <div class="ad-card-title"><i class="fa fa-calendar-check-o"></i> All Land Bookings — booking table</div>
        <div class="ad-card-sub">Cols: S_ID, S_NAME, area, D_NO, SUNO, U_NAME, U_NUMBER, STS, B_fess, C_Type</div>
      </div>
    ...
    <div class="ad-table-wrap">
      <table class="ad-table">
        <thead>
          <tr>
            <th>S_ID</th><th>Seller</th><th>Area</th><th>Doc No</th><th>Survey No</th>
            <th>Buyer Name</th><th>Buyer Mobile</th><th>Buyer Email</th>
            <th>Type</th><th>Fee</th><th>Status</th>
          </tr>
        </thead>
        <tbody id="bkBody">
        ...
        <tr>
          ...
          <td style="font-size:.8rem;font-weight:600"><%=rs.getString("area")!=null?rs.getString("area"):"—"%></td>
        ...
        <%}if(!found){%>
        <tr><td colspan="11"><div class="ad-empty"><i class="fa fa-calendar-check-o"></i><h3>No land bookings yet</h3><p>booking table is empty</p></div></td></tr>
        <%}%>
        </tbody>
      </table>
    </div>
